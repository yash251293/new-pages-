...
import { loginUser } from "@/lib/api" // Added
  ...
  const onEmailLoginSubmit: SubmitHandler<EmailLoginFormValues> = (data) => {
    ...
    try {
      ...
      if (response.token && response.user) {
        ...
        auth.login(response.token, response.user); // Update auth context
        router.push('/feed'); // Redirect to a protected route
      } else {
        // This case might not be reached if loginUser throws an error for bad responses
        ...
        <div className="w-1/2 flex flex-col justify-center bg-white p-12 min-h-full">
          ...
          <div className="flex mb-6 bg-gray-100 rounded-lg p-1">
            <button
              type="button"
              onClick={() => {
                if (isLoading) return; // Prevent switching if email login is loading
          ...
          <form className="space-y-6" onSubmit={loginMethod === 'email' ? handleEmailSubmit(onEmailLoginSubmit) : (e) => e.preventDefault()}>
            {loginMethod === 'email' ? (
              // Email Login Form
              <>
                <div>
          ...
          <div className="flex gap-8 mb-6 justify-center">
            <button className="hover:opacity-70 transition-opacity cursor-pointer" disabled={isLoading}>
