...
// import { Logo } from "@/components/logo"; // Logo component not used in the new UI's form area, can be removed if not needed elsewhere
...
import { registerUser, loginUser } from "@/lib/api"; // Assuming these are correct
import { toast } from "sonner"; // Assuming sonner is used for toasts
import { useAuth } from "@/context/AuthContext"; // Assuming AuthContext is used

// Adjusted Zod schema to include new fields and match old logic's requirements
const formSchema = z.object({
  user_type: z.enum(["individual", "company"]),
  full_name: z.string().optional(), // Required for individual, handled by refine
  company_name: z.string().optional(), // Required for company, handled by refine
  industry: z.string().optional(), // Optional for company
  company_size: z.string().optional(), // Optional for company
  gender: z.string().optional(), // New field, make optional or required based on product decision for individuals
  birth_date: z.string().optional(), // New field, make optional or required. Ensure backend can handle date string.
  ...
  confirmPassword: z.string(),
    ...
    // Make gender required only for individuals if that's the intent
    if (data.user_type === 'individual' && !data.gender) {
        return false;
    }
    return true;
}, {
    ...
    // Make birth_date required for individuals if that's the intent
    // Also, consider actual date validation if needed (e.g., not in future)
    if (data.user_type === 'individual' && !data.birth_date) {
        return false;
    }
    return true;
}, {
    message: "Date of Birth is required for individual users.",
    path: ["birth_date"],
...
function SignUpContent() {
  ...
  const onSubmit: SubmitHandler<SignUpFormValues> = async (data) => {
    ...
    if (apiData.user_type === 'individual') {
      ...
      delete apiData.birth_date; // Companies don't have DOB/Gender
    }

    try {
      const response = await registerUser(apiData); // registerUser needs to handle the new fields
      ...
      try {
        ...
        if (loginResponse && loginResponse.token && loginResponse.user) {
          ...
          // Redirect to verify-email, pass userType if needed by that page
  ...
  return (
    // This is the outer div from new-signup-page.tsx
    // <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="flex w-full max-w-6xl mx-auto rounded-2xl shadow-xl overflow-hidden border">
        {/* Left Side - Image and Text */}
        <div className="w-1/2 flex-col items-center justify-center bg-[#FFFCF6] p-12 hidden md:flex"> {/* Added hidden md:flex from old page */}
          <Image
            src="/imagex.png" // Assuming this path is correct and image exists in public
        ...
        {/* Right Side - Card */}
        <div className="w-full md:w-1/2 flex flex-col justify-center bg-white p-8 sm:p-12 min-h-full"> {/* Added p-8 sm:p-12 from old page */}
          ...
          <div className="flex justify-center space-x-2 sm:space-x-4 mb-6">
            <Button
              type="button" // Important for RHF
              onClick={() => handleUserTypeChange('company')}
              variant={userType === 'company' ? 'default' : 'outline'} // Use RHF's userType
              className={`px-4 py-2 sm:px-6 sm:py-2 rounded-lg font-medium ${
                userType === 'company'
                  ? 'bg-brand-blue text-white'
                  : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`} // Adjusted classes slightly
            >
              For Company
            </Button>
            <Button
              ...
              variant={userType === 'individual' ? 'default' : 'outline'} // Use RHF's userType
              className={`px-4 py-2 sm:px-6 sm:py-2 rounded-lg font-medium ${
                userType === 'individual'
                  ? 'bg-brand-blue text-white'
                  : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6"> {/* Applied space-y from new UI */}
            {userType === 'company' ? (
              <>
                <div>
              ...
              <>
                ...
                <div>
                  ...
                  <Controller
                    name="gender"
                    control={control}
                    rules={{ required: userType === 'individual' ? "Gender is required" : false }}
                    render={({ field }) => (
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input
                      id="birth_date"
                      type="date"
                      {...register("birth_date", { required: userType === 'individual' ? "Date of Birth is required" : false })}
            ...
            <div>
             ...
             <div> {/* Confirm Password from old form */}
          ...
          <div className="flex gap-4 sm:gap-8 mb-6 justify-center"> {/* Adjusted gap */}
          ...
          <div className="mt-6 sm:mt-8 text-center"> {/* Adjusted margin */}
...
// Main export wrapped in Suspense, as in the old page
export default function SignUpPage() { // Renamed for clarity if needed, or keep as SignUpPage
  return (
    <div className="min-h-screen flex items-center justify-center bg-white"> {/* This outer div is from new-signup-page */}
      <Suspense fallback={<div className="text-center p-10">Loading signup form...</div>}>
        <SignUpContent />
      </Suspense>
    </div>
  );
